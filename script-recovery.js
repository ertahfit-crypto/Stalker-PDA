// Local Persistence & Recovery Mode

// Check for saved user and set global state
const savedUser = JSON.parse(localStorage.getItem('pda_user'));
if (savedUser) {
    window.currentUser = savedUser;
    console.log('👤 Загружен сохраненный пользователь:', savedUser.username);
    
    // Skip login screen and go directly to main interface
    ...
    // Set default identity if no saved user
    window.currentUser = {
        username: 'erik',
        level: 'Сталкер',
        avatar: 'https://picsum.photos/seed/erik/100/100.jpg'
    };
    console.log('👤 Использован профиль по умолчанию');
}

// Simplified messaging with error handling
window.sendMessage = async function() {
    ...
    if (!window.db || !window.dbFunctions) {
        ...
        if (typeof showGlitchEffect === 'function') {
            showGlitchEffect('sendBtn', 'ОФФЛАЙН');
        }
        return;
    }
...
// Display Message with Local Identity
